...
public class Board
{
    ...
    private Cell[,] bot_cells;
    private int matched_count = 0;

    private Transform m_root;

    private int m_matchMin;
    List<NormalItem.eNormalType> typeList = new List<NormalItem.eNormalType>();
    ...
    private void CreateBoard()
    {
        ...
        {
            GameObject go = GameObject.Instantiate(prefabBG);
            go.transform.position = origin + new Vector3(x - 0.5f, -2, 0f);
            go.transform.SetParent(m_root);
    ...
    {
        for (int i = 0; i < bot_cells.Length; i++)
        {
            if (bot_cells[i, 0].Item == null)
            {
                Item item = cell.Item;
                cell.Free();
                bot_cells[i, 0].Free();
                bot_cells[i, 0].Assign(item);

                item.View.DOMove(bot_cells[i, 0].transform.position, 0.6f)
                    .OnComplete(() => DOVirtual.DelayedCall(0.6f, () => CheckBotCellsMatched()));
                item.View.DOScale(Vector3.one * 1.5f, 0.3f)
                    .OnComplete(() => item.View.DOScale(Vector3.one, 0.3f));
            }
        }
    }
    public void CheckBotCellsMatched()
    {
        for (int i = 0; i < bot_cells.Length - 2; i++)
        {
            for (int j = i + 1; j < bot_cells.Length - 1; j++)
            {
                for (int k = j + 1; k < bot_cells.Length; k++)
                {
                    if (bot_cells[i, 0].Item.IsSameType(bot_cells[j, 0].Item) &&
                        bot_cells[j, 0].Item.IsSameType(bot_cells[k, 0].Item))
                    {
                        bot_cells[i, 0].ExplodeItem();
                        bot_cells[j, 0].ExplodeItem();
                        bot_cells[k, 0].ExplodeItem();
                        matched_count++;
                        Debug.Log(matched_count);
                    }
                }
            }
        }
        if (IsBotCellsFull()) matched_count = -1;
    }
    void GetTypeList()
    {
        ...
        {
            int rnd = UnityEngine.Random.Range(0, list.Count);
            for (int j = 0; j < 6; j++) typeList.Add(list[rnd]);
            list.RemoveAt(rnd);
        }
    ...
    NormalItem.eNormalType GetRandomNormalType()
    {
        int rnd = UnityEngine.Random.Range(0, typeList.Count);
        NormalItem.eNormalType result = typeList[rnd];
        typeList.RemoveAt(rnd);

        return result;
    }
    public int CheckMatchedCount()
    {
    ...
    public bool IsBotCellsFull()
    {
        foreach (Cell cell in bot_cells)
        {
